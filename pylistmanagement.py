...
def column_operation(listas,method="concat"):
    if len(listas) == 1:
        return(print("At least two lists are expected to be received.\n"))
    for m in listas:
        if len(m)==0:
...
def filtering(dataf,list1,filter,method=="equal"):
    if method == "equal":    
